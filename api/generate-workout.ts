  ...
  try {
    ...
    if (optimizerRecommendations) {
      ...
      const taperedRx = optimizerRecommendations.taperedRepScheme
        ? ` TAPERED SETS (BINDING): For EACH main exercise use LEAD ${optimizerRecommendations.taperedRepScheme.leadSets}×${optimizerRecommendations.taperedRepScheme.leadReps} @ RPE ${optimizerRecommendations.taperedRepScheme.leadRPE}, then TAPER ${optimizerRecommendations.taperedRepScheme.taperSets}×${optimizerRecommendations.taperedRepScheme.taperReps} @ RPE ${optimizerRecommendations.taperedRepScheme.taperRPE}. List lead sets first, then taper. Total ~${optimizerRecommendations.taperedRepScheme.totalReps} reps, Frederick ~${Math.round(optimizerRecommendations.taperedRepScheme.totalFrederickLoad)}. WEIGHT REQUIRED: If 1RM data exists, MUST include weightLbs and percentOf1RM for ALL exercises (lead AND taper). Lead: ${optimizerRecommendations.intensityRange.min}–${optimizerRecommendations.intensityRange.max}% 1RM. Taper: lighter load matching lower RPE but NEVER below 60% 1RM. Round to nearest 5 lbs.`
        : '';
      optimizerContext = `⚡ OPTIMIZER (BINDING — DO NOT OVERRIDE WITH GENERIC DEFAULTS): ${optimizerRecommendations.sessionVolume} working sets, ${optimizerRecommendations.repScheme}, ${optimizerRecommendations.intensityRange.min}-${optimizerRecommendations.intensityRange.max}% 1RM, ${optimizerRecommendations.exerciseCount.min}-${optimizerRecommendations.exerciseCount.max} exercises. Rationale: ${optimizerRecommendations.rationale}${metLoad}${fatigueLoad}${taperedRx}${peakForceRx} COMPLIANCE: Every exercise must match the optimizer's sets/reps/intensity. Do NOT default to 3×10 @ 70%.`;
    }
