  ...
  try {
    ...
    if (optimizerRecommendations) {
      ...
      const taperedRx = optimizerRecommendations.taperedRepScheme
        ? ` TAPERED SETS (BINDING): For EACH main exercise use LEAD ${optimizerRecommendations.taperedRepScheme.leadSets}×${optimizerRecommendations.taperedRepScheme.leadReps} @ RPE ${optimizerRecommendations.taperedRepScheme.leadRPE}, then TAPER ${optimizerRecommendations.taperedRepScheme.taperSets}×${optimizerRecommendations.taperedRepScheme.taperReps} @ RPE ${optimizerRecommendations.taperedRepScheme.taperRPE}. List lead sets first, then taper. Total ~${optimizerRecommendations.taperedRepScheme.totalReps} reps, Frederick ~${Math.round(optimizerRecommendations.taperedRepScheme.totalFrederickLoad)}. WEIGHT REQUIRED: If 1RM data exists, MUST include weightLbs and percentOf1RM for ALL exercises (lead AND taper). Lead: ${optimizerRecommendations.intensityRange.min}–${optimizerRecommendations.intensityRange.max}% 1RM. Taper: lighter load matching lower RPE. Round to nearest 5 lbs.`
        : '';
